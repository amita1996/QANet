...
def build_block_3d(in_channels, out_channels):
    return nn.Sequential(
        nn.Conv3d(in_channels, out_channels, kernel_size=3, stride=1, padding=1),
        nn.BatchNorm3d(out_channels),
        # nn.InstanceNorm3d(out_channels),  # Using InstanceNorm instead of BatchNorm
        nn.LeakyReLU(),
        nn.MaxPool3d(kernel_size=2, stride=2),
    )


class Rib3D(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        super().__init__()
        if mid_channels is None:
            mid_channels = in_channels * 3
        self.left = build_block_3d(in_channels, out_channels)
        self.right = build_block_3d(in_channels, out_channels)
        self.mid = build_block_3d(mid_channels, out_channels)
...
class RibCage3D(nn.Module):
    def __init__(self, *args, channels, **kwargs):
        super().__init__(*args, **kwargs)
        self.rib1 = Rib3D(channels[0], channels[1], mid_channels=channels[0] * 2)
        self.rib2 = Rib3D(channels[1], channels[2])
        self.rib3 = Rib3D(channels[2], channels[3])
        self.rib4 = Rib3D(channels[3], channels[4])
...
if __name__ == '__main__':
    ...
    rib_channels3d = [1, 32, 64, 128, 256]
    rib_model3d = RibCage3D(channels=rib_channels)


    # sanity check
    out = (rib_model3d(torch.randn(2, 1, 32, 256, 256), torch.randn(2, 1, 32, 256, 256)))
    ...
    #
    #
    # for model, name in zip([rib_model, naive_model, siamese_model, rib_model3d], ['RibCage', 'Naive Model', 'Siamese Model', 'RibCage3D']):
